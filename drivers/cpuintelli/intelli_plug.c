...
static void __cpuinit intelli_plug_power_resume(struct power_suspend *handler)
{
  __intelli_plug_resume();
}
...
#elif defined CONFIG_HAS_EARLYSUSPEND
static void intelli_plug_power_suspend(struct early_suspend *handler)
{
  __intelli_plug_suspend();
}

static void __cpuinit intelli_plug_power_resume(struct early_suspend *handler)
{
  __intelli_plug_resume();
}
